...
int main(int argc, char **argv) {
  ...
  try {
    ...
    sampling_options.add_options()
      ...
      ("read2,2", po::value(&path2),
       "Path to FASTA file. Has to be paired with the first.")
      ("sample,k", po::value(&k)->required(),
       "How many sequences to sample. [REQUIRED]")
      ("size,n", po::value(&n),
       "How many sequences there are in the FASTA files. "
       "If not specified this will be determined by reading the first FASTA file prior to sampling.")
      ("seed,s", po::value(&seed),
       "Seed to initialize the pseudo random number generator.")
      ("onethread", po::bool_switch(&single_threaded),
       "Run single threaded. By default, two threads are used when processing paired FASTA files.");
      ;
  ...
  if (vm.count("help")) {
    ...
    cout << "This program samples sequences without replacement from one or a pair of FASTA\n"
         ...
         << endl;
    cout << "Sequences sampled from the first (or only) FASTA file are written to the\n"
            "standard output stream, while sequences sampled from the second FASTA file are\n"
            "written to the standard error stream.\n"
         << endl;
    cout << "Note that the input FASTA files may also be compressed using gzip or bzip2.\n"
            "Depending on their filename ending (.gz or .bz2), they will be uncompressed on\n"
            "the fly.\n"
         << endl;
    cout << "Note that the sequences of the input FASTA are expected to be one a single line.\n"
         << endl;
    cout << desc << "\n";
    return EXIT_SUCCESS;
  }
